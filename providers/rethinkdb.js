...
module.exports = class extends Provider {
	...
	constructor(...args) {
		super(...args);
		this.db = rethink(this.client.options.providers.rethinkdb || { db: 'osbot' });
	}
   ...
   */
	async removeValue(table, doc) {
		...
		if (typeof doc === 'object') {
			return this.db
				.table(table)
				.replace(this.db.row.without(doc))
				.then(resolvePromise);
		}
		// 'channels.modlog'
		if (typeof doc === 'string') {
			...
			return this.db
				.table(table)
				.replace(this.db.row.without(object))
				.then(resolvePromise);
		}
   ...
   */
	updateArrayByIndex(table, id, uArray, index, doc) {
		return this.db
			...
			.get(id)
			.update({
				[uArray]: this.db.row(uArray).changeAt(
					index,
					this.db
						.row(uArray)
						.nth(index)
   ...
   */
	updateArrayByID(table, id, uArray, index, doc) {
		return this.db
			.table(table)
			.get(id)
			.update({ [uArray]: this.db.row(uArray).map(da => this.db.branch(da('id').eq(index), da.merge(doc), da)) })
			.then(resolvePromise);
	}
   ...
   */
	removeFromArrayByIndex(table, id, uArray, index) {
		return this.db
			.table(table)
			.get(id)
			.update({ [uArray]: this.db.row(uArray).deleteAt(index) })
			.then(resolvePromise);
	}
   ...
   */
	removeFromArrayByID(table, id, uArray, index) {
		return this.db
			.table(table)
			.get(id)
			.update({ [uArray]: this.db.row(uArray).filter(it => it('id').ne(index)) })
			.then(resolvePromise);
	}
   ...
   */
	getFromArrayByID(table, id, uArray, index) {
		return this.db
			.table(table)
			.get(id)(uArray)
			.filter(rethink.row('id').eq(index))
