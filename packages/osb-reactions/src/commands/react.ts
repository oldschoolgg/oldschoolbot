...
export const reactCommand = defineCommand({
	...
	options: [
		{
			type: 'Subcommand',
			name: 'add',
			description: 'Add an emoji thats reacted when you are pinged or replied to.',
			options: [
				{
					type: 'String',
					name: 'emoji',
	...
	run: async ({ options, user }) => {
		if (user.leaguesPointsTotal < 30_000) {
			return 'You are not worthy. You need at least 30,000 League Points to be able to have a reaction.';
		}
		if (options.add) {
			...
			return "Done. If your reaction emoji isn't working, try changing it to a new one that OSB Reactions can view, or a built-in emoji.";
		}
		if (options.remove) {
			await user.update({
				react_emoji_id: null
