...
import { type FetchingStrategyOptions, managerToFetchingStrategyOptions } from '../context/IContextFetchingStrategy.js';
	...
	public async destroy(options: Omit<WebSocketShardDestroyOptions, 'recover'> = {}) {
		...
		for (const [shardId, worker] of this.#workerByShardId.entries()) {
			...
			promises.push(
				...
				new Promise<void>(resolve => this.destroyPromises.set(shardId, resolve)).then(async () =>
					worker.terminate()
				)
			);
			worker.postMessage(payload);
