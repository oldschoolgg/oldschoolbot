import type { Item } from '@/osrs/item.js';
import { toKMB } from '@/osrs/utils.js';
import { WebItems } from '@/osrs/WebItems.ts';
...
export class Bank {
	...
	private resolveItemID(item: ItemResolvable): number {
		...
		if (typeof item === 'string') return WebItems.getId(item);
		return item.id;
	}
	...
	private makeFromInitialBank(initialBank?: number[] | Record<string, number> | Bank | Map<number, number>) {
		...
		for (const k in initialBank) {
			...
			const n = +k;
			const id = Number.isInteger(n) && WebItems.has(n) ? n : WebItems.getId(k);
			if (id) {
				out.set(id, qty);
			}
		}
	...
	public add(item: string | number | IntKeyBank | Bank | Item | undefined, quantity = 1): Bank {
		...
		if (typeof item === 'string') {
			return this.addItem(WebItems.getId(item), quantity);
		}
	...
	public remove(item: string | number | ItemBank | Bank, quantity = 1): Bank {
		...
		if (typeof item === 'string') {
			return this.removeItem(WebItems.getId(item), quantity);
		}
	...
	public items(): [Item, number][] {
		...
		for (const [key, val] of this.map.entries()) {
			if (val < 1) continue;
			const item = WebItems.get(key).item;
			if (!item) {
				continue;
			}
			arr.push([item, val]);
