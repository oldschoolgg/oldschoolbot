...
describe("util", () => {
	...
	test("getOSItem", () => {
		...
		expect(() => getOSItem("Non-existant item")).toThrowError(
	...
	test("getUserFoodFromBank", () => {
		expect(
			getUserFoodFromBank({ [itemID("Shark")]: 1 }, 500)
		).toStrictEqual(false);
		expect(
			getUserFoodFromBank({ [itemID("Shark")]: 100 }, 500)
		).toStrictEqual({
			[itemID("Shark")]: 25,
		});
		expect(
			getUserFoodFromBank(
				{ [itemID("Shark")]: 30, [itemID("Tuna")]: 20 },
				750
			)
		).toStrictEqual({
			[itemID("Tuna")]: 20,
