...
describe('Dice Command', async () => {
	test('Lose dice', async () => {
		...
		expect(result).toContain('rolled **11** on the percentile dice, and you lost -100m GP.');
	...
	test('Won dice', async () => {
		...
		await user.gpMatch(100_000_000);
		const result = await user.runCommand(gambleCommand, { dice: { amount: '100m' } });
		expect(result).toContain('rolled **91** on the percentile dice, and you won 100m GP.');
