...
2. Install the yarn dependency: `npm install --global yarn`
3. Make a config file from the example: `cp src/config.example.ts src/config.ts`
4. Edit this new `config.ts` file, input your bot token and Postgres database details; be sure to read the comments for any additional instructions.
...
6. Create a file called `.env` in your project's root directory with the content
...
7. Run `npx prisma generate` to generate the Prisma client files and load the DSN from #6
8. Run `npx prisma db push` to create the tables on the database referenced in .env
9. Run `yarn build` - then run `yarn start`. In the future, you can type only `yarn start` to start the bot.
