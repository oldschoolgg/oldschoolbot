	...
	async run(msg, [board = 'gp']) {
		if (board === 'gp') {
			const users = this.client.users
				...
				.sort((a, b) => Object.keys(b.settings.get('pets')).length - a.settings.get('GP')).first(10);
			...
			const leaderboard = users.map(({ username, settings }) => `**${escapeMarkdown(username)}** has ${Object.keys(settings.get('pets')).length} GP `);
