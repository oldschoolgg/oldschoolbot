	...
	constructor(...args) {
		...
		this.language = {
			...
			*/
			...
			AUTO_UPDATE_NO_RSN: (prefix) => `You must have an RSN set to Autoupdate. Use \`${prefix}setrsn <username>\``,
			AUTO_UPDATE: (state) => `Turned Auto Updating for your account ${state ? '**on**.' : '**off**.'}`,
			...
			HCIM_TWEETS_ARENT_ENABLED: "HCIM Death Tweets aren't enabled, so you can't disable them.",
			...
			RSN_NOT_SET: (prefix) => `You don't have an RSN set. You can set one like this: \`${prefix}setrsn <username>\``,
			...
			STREAMER_TWEETS_ARENT_ENABLED: "Streamer Tweets aren't enabled, so you can't disable them.",
			...
			TN_NO_STREAMER: (prefix) => `Please include which streamer you want to add, like this: \`${prefix}tn add B0aty\``,
			TN_INVALID_STREAMER: 'That streamer doesn\'t look like a Old School RuneScape Streamer to me. You can request they be added in the support server.',
			TN_ALREADY_ENABLED_STREAMER: 'Twitch Notifications are already enabled for this streamer.',
			TN_ADDED_STREAMER: (name) => `Successfully added ${name}. You will receive a message here when they go live.`,
			TN_NO_STREAMER_REMOVE: (prefix) => `Please include which streamer you want to remove, like this: \`${prefix}tn remove B0aty\``,
