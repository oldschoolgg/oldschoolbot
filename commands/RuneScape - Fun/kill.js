...
module.exports = class extends Command {
	...
	Raids(quantity) {
		if (quantity <= 100) {
			...
			const loot = {
				Dexterous_prayer_scroll: 0,
				Arcane_prayer_scroll: 0,
				Dragon_sword: 0,
				Dragon_harpoon: 0,
				Dragon_thrownaxe: 0,
				Twisted_buckler: 0,
				Dragon_hunter_crossbow: 0,
				Dinhs_bulwark: 0,
				Ancestral_hat: 0,
				Ancestral_robe_top: 0,
				Ancestral_robe_bottom: 0,
				Dragon_claws: 0,
				Elder_maul: 0,
				Kodai_insignia: 0,
				Twisted_bow: 0,
			...
			let total_value = 0;
			const price_map = {
				Dexterous_prayer_scroll: 75000000,
				Arcane_prayer_scroll: 10000000,
				Dragon_sword: 500000,
				Dragon_harpoon: 5000000,
				Dragon_thrownaxe: 400000,
				Twisted_buckler: 16000000,
				Dragon_hunter_crossbow: 60000000,
				Dinhs_bulwark: 12000000,
				Ancestral_hat: 18000000,
				Ancestral_robe_top: 100000000,
				Ancestral_robe_bottom: 79000000,
				Dragon_claws: 73000000,
				Elder_maul: 47000000,
				Kodai_insignia: 105000000,
				Twisted_bow: 1000000000,
				Olmlet: 0
			};
			for (let i = 0; i < quantity; i++) {
				if (roll(25)) {
					...
					switch (true) {
						case number < 23.81:
							loot.Dexterous_prayer_scroll++;
							break;
						case number < 47.61:
							loot.Arcane_prayer_scroll++;
							break;
						case number < 53.56:
							loot.Dragon_sword++;
							break;
						case number < 59.51:
							...
							break;
						case number < 65.46:
							loot.Dragon_thrownaxe++;
							break;
						case number < 70.22:
							loot.Twisted_buckler++;
							break;
						case number < 74.98:
							loot.Dragon_hunter_crossbow++;
							break;
						case number < 78.55:
							loot.Dinhs_bulwark++;
							break;
						case number < 82.12:
							...
							break;
						case number < 85.69:
							loot.Ancestral_robe_top++;
							break;
						case number < 89.26:
							loot.Ancestral_robe_bottom++;
							break;
						case number < 92.83:
							loot.Dragon_claws++;
							break;
			...
			for (var key in loot) {
				display_loot.push(`**${key.replace(/_/g, ' ')}**: ${loot[key].toLocaleString()} `);
				total_value += price_map[key] * loot[key];
			}
			display_loot.push(`\n**Total Value:** ${total_value.toLocaleString()} GP`);
			display_loot.push(`**GP/HR (30-Min Solo Raids):** ${(total_value / (quantity / 2)).toLocaleString()} GP`);
			display_loot.push(`**Total Hours**: ${(quantity / 2).toLocaleString()}`);
			return display_loot.join('\n');
		}
	}
	...
	Bandos(quantity) {
		...
		if (quantity <= 250) {
			...
			display_loot.push(`\n**Total Value:** ${total_value.toLocaleString()} GP`);
			display_loot.push(`**GP per Kill:** ${Math.round(total_value / quantity).toLocaleString()} GP`);
			return display_loot.join('\n');
		}
	}
