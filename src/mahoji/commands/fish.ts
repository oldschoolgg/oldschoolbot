...
export const fishCommand = defineCommand({
	...
	run: async ({ options, user, channelId }) => {
		...
		if (
			spot.name !== 'Lobster' &&
			user.usingPet('Patricia') &&
			user.hasEquipped('Old crab cage') &&
			!user.cl.has('Partycrab')
		) {
			response += `\n\nPatricia keeps circling the cage, but whatever you're hoping to coax out of it won't answer here.`;
		}
		if (result.suppliesToRemove.length > 0) {
