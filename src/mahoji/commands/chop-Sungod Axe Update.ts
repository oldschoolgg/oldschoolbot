...
export const chopCommand: OSBMahojiCommand = {
    ...
    run: async ({
        ...
        if (user.hasEquipped(['Sun-God Axe'])) {
            const [predeterminedTotalTime] = determineWoodcuttingTime({
                quantity,
                user,
                log,
                axeMultiplier: 16.67, // 2x Drygore's multiplier (1.2*16.67 = 20 with 1.2 from powerchop)
                powerchopping: true, // No resources received & multiplier adjusted
                forestry: forestry_events,
                woodcuttingLvl: wcLvl
            });
            axeMultiplier = 16.67;
            boosts.pop();
            boosts.push('**20x** success multiplier for Sun-God Axe [Equipped] (no resources received)');
        }
        else if (user.hasEquippedOrInBank(['Drygore axe'])) {
            const [predeterminedTotalTime] = determineWoodcuttingTime({
                quantity,
                user,
                log,
        ...
        let response = `${minionName(user)} is now chopping ${log.name} until your minion ${quantity ? `chopped ${newQuantity}x or gets tired` : 'is satisfied'
            }, it'll take ${quantity
