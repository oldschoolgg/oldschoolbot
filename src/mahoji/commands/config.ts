...
async function favFoodConfig(user: User, itemToAdd: string | undefined, itemToRemove: string | undefined) {
	...
	return `Your current favorite food is: ${currentFavorites.map(itemNameFromID).join(', ')}.`;
}
...
export const configCommand: OSBMahojiCommand = {
	...
	run: async ({
		...
		if (options.user) {
			const { toggle, combat_options, set_rsn, bg_color, bank_sort, favorite_alchs, favorite_food } =
				options.user;
			if (toggle) {
