...
export async function igneCommand(
	...
	const instance = new BossInstance({
		...
		itemCost: async (data) => {
			...
			let brewsNeeded =
				Math.max(1, 10 - Math.max(1, Math.ceil((kc + 1) / 30))) + 2;
			if (data.solo) {
			...
			return userBank.has(heatResBank.bank)
				? heatResBank
				: hasEnough
				? foodBank
				: defaultBank;
		...
		food: () => new Bank(),
		settingsKeys: [
			ClientSettings.EconomyStats.IgnecarusCost,
			ClientSettings.EconomyStats.IgnecarusLoot,
		],
		channel,
		activity: "Ignecarus",
		...
		const embed = new MessageEmbed()
			.setDescription(
				`Your team is off to fight ${
					instance.quantity
				}x Ignecarus. The total trip will take ${formatDuration(
					instance.duration
				)}.
