...
export async function slotsCommand(
	...
	const amount = mahojiParseNumber({ input: slotsamount, min: 20_000_000, max: 1_000_000_000 });
...
${buttonsData.map(b => `${b.name}: ${b.mod(1)}x`).join('\n')}`;
	...
	await KlasaUser.addItemsToBank({ items: new Bank().add('Coins', amountReceived), collectionLog: false });
	await updateGPTrackSetting(client, ClientSettings.EconomyStats.GPSourceSlots, amountReceived - amount);
	await updateGPTrackSetting(KlasaUser, UserSettings.GPSlots, amountReceived - amount);
