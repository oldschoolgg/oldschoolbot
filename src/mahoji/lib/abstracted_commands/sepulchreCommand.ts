...
export async function sepulchreCommand(user: MUser, channelId: string, fletching?: number) {
	...
	}-${completableFloors[completableFloors.length - 1].number}, the trip will take ${await formatTripDuration(user, tripLength)}, with each lap taking ${formatDuration(lapLength)}.`;
