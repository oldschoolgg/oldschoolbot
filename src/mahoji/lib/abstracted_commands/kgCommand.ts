...
export async function kgCommand(
	...
	const instance = new BossInstance({
		...
		gearSetup: "melee",
		itemCost: async (data) => {
			const brewsNeeded = data.baseFood
				.multiply(data.kills)
				.items()
				.find(([item]) => item.name === "Saradomin brew(4)")![1];

			// Find if the user has enough brews&restores with enhanced ones
			// If they do not, default back to just normals
			const { foodBank, hasEnough } = brewRestoreSupplyCalc(
				data.user,
				brewsNeeded
			);
			const defaultBank = data.baseFood
				.multiply(data.kills)
				.add("Coins", gpCostPerKill(data.user) * data.kills);

			return hasEnough
				? foodBank.add("Coins", gpCostPerKill(data.user) * data.kills)
				: defaultBank;
		},
	...
	try {
		...
		const embed = new MessageEmbed()
			.setDescription(
				`${
					type === "solo" ? "You approach" : "Your group approaches"
				} the Kings' chambers, and ${
...
`
		...
		return {
			embeds: [embed],
			content:
				instance.boosts.length > 0
					? `**Boosts:** ${instance.boosts.join(", ")}.`
					: undefined,
		};
	} catch (err: any) {
