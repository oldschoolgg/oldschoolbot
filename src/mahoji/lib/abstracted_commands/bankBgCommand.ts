...
export async function bankBgCommand(interaction: SlashCommandInteraction, user: KlasaUser, name: string) {
	...
	let allAccounts: string[] = [...user.settings.get(UserSettings.IronmanAlts)];
	const main = user.settings.get(UserSettings.MainAccount);
	if (main) allAccounts.push(main);
	if (
		user.settings.get(UserSettings.BitField).includes(BitField.isModerator) ||
