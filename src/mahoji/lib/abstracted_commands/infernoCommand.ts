...
const minimumRangeAttackStat = sumArr(
	minimumRangeItems.map((i) => i.equipment!.attack_ranged)
);
const minimumRangeMagicDefenceStat =
	sumArr(minimumRangeItems.map((i) => i.equipment!.defence_magic)) - 10;
...
const minimumMageAttackStat = sumArr(
	minimumMageItems.map((i) => i.equipment!.attack_magic)
);
const minimumMageMagicDefenceStat =
	sumArr(minimumMageItems.map((i) => i.equipment!.defence_magic)) - 10;
...
function consumableCost({
	...
	const iceBarrageRunes = new Bank()
		.add("Death rune", 4)
		.add("Blood rune", 2);
	const bloodBarrageRunes = new Bank()
		.add("Death rune", 4)
		.add("Blood rune", 4)
		.add("Soul rune");
	const elderBarrageRunes = new Bank()
		.add("Elder rune", 1)
		.add("Blood rune", 12)
		.add("Death rune", 8);
	...
	cost.add(
		bloodBarrageRunes.multiply(Math.floor(bloodBarragePerHour * hours))
	);
	cost.add(iceBarrageRunes.multiply(Math.floor(iceBarragePerHour * hours)));
	if (isEmergedZuk) {
		cost.add(
			elderBarrageRunes.multiply(Math.floor(elderBarragePerHour * hours))
		);
...
function basePreZukDeathChance(_attempts: number) {
	...
	let chance = Math.floor(
		100 - (Math.log(attempts) / Math.log(Math.sqrt(45))) * 47
	);
...
function baseZukDeathChance(_attempts: number) {
	...
	const chance = Math.floor(
		150 - (Math.log(attempts) / Math.log(Math.sqrt(25))) * 39
	);
...
function baseDuration(_attempts: number, isEmergedZuk: boolean) {
	...
	let chance = Math.floor(
		150 - (Math.log(attempts) / Math.log(Math.sqrt(65))) * 45
	);
	if (attempts < 20) chance += 20 - attempts;
	let res = Math.min(
		Time.Hour * 2.5,
		Math.max(Time.Minute * 40, chance * (Time.Minute * 2.9))
	);
...
async function infernoRun({
	...
	const duration = new PercentCounter(
		baseDuration(attempts, isEmergedZuk),
		"time"
	);
	const zukDeathChance = new PercentCounter(
		baseZukDeathChance(attempts),
		"percent"
	);
	const preZukDeathChance = new PercentCounter(
		basePreZukDeathChance(attempts),
		"percent"
	);
	const emergedZukDeathChance = new PercentCounter(
		baseEmergedZukDeathChance(emergedAttempts),
		"percent"
	);
	...
	if (
		isEmergedZuk &&
		!user.settings.get(UserSettings.SacrificedBank)[itemID("Infernal cape")]
	) {
	...
	if (!hasSkillReqs) {
		return `You not meet skill requirements, you need ${Object.entries(
			skillReqs
		)
	...
	if (!isEmergedZuk) {
		duration.add(
			(rangeGear.hasEquipped("Armadyl chestplate") &&
				rangeGear.hasEquipped("Armadyl chainskirt")) ||
				(rangeGear.hasEquipped("Pernix body") &&
					rangeGear.hasEquipped("Pernix chaps")),
			-3,
			"Armadyl/Pernix"
		);
		duration.add(
			(mageGear.hasEquipped("Ancestral robe top") &&
				mageGear.hasEquipped("Ancestral robe bottom")) ||
				(mageGear.hasEquipped("Virtus robe top") &&
					mageGear.hasEquipped("Virtus robe legs")),
			-4,
			"Ancestral/Virtus"
		);
	}
	const hasDivine =
		rangeGear.hasEquipped("Divine spirit shield") ||
		mageGear.hasEquipped("Divine spirit shield");
	preZukDeathChance.add(hasDivine, -12, "Divine");
	emergedZukDeathChance.add(hasDivine, -9, "Divine");
	preZukDeathChance.add(
		!hasDivine &&
			(rangeGear.hasEquipped("Elysian spirit shield") ||
				mageGear.hasEquipped("Elysian spirit shield")),
		-5,
		"Ely"
	);
	duration.add(
		mageGear.hasEquipped("Virtus book", true, true),
		-7,
		"Virtus book"
	);
	if (isEmergedZuk) {
		duration.add(
			user.hasItemEquippedOrInBank("Dwarven warhammer"),
			-7,
			"DWWH"
		);
	}
	...
	preZukDeathChance.add(
		rangeGear.hasEquipped("Justiciar faceguard"),
		-5,
		"Just. faceguard"
	);

	const hasSuffering =
		rangeGear.hasEquipped("Ring of suffering (i)") ||
		mageGear.hasEquipped("Ring of suffering (i)");
	...
	if (isEmergedZuk) {
		if (
			!["Dragon dart", "Rune dart", "Amethyst dart"].includes(
				dartItem.name
			)
		) {
	...
	for (const [name, setup, weapons] of [
		...
		if (
			...
			return `You need one of these weapons in your ${name} setup: ${Object.keys(
				weapons
			).join(", ")}.`;
		}
	}
	...
	if (isEmergedZuk) {
		...
		if (
			!resolveItems([
				...
				"Torva boots",
				"Torva gloves",
			]).every((i) => allMeleeGearItems.includes(i))
		) {
			return "You need stronger melee armor! TzKal-Zuk will crush you. Try getting Torva or Gorajan.";
		}
		const hasTzkalCape = [meleeGear, rangeGear, mageGear].some((s) =>
			s.hasEquipped("Tzkal cape")
		);
		...
		duration.add(
			allItems.includes(itemID("Ignis ring(i)")),
			-5,
			"Ignis ring(i)"
		);
		emergedZukDeathChance.add(
			user.skillLevel(SkillsEnum.Defence) === 120,
			-10,
			"120 Defence"
		);
		...
		if (emergedKC > 0) {
			...
			emergedZukDeathChance.add(
				true,
				0 - effectiveKC * 7.5,
				`${effectiveKC} Emerged KC`
			);
		}
	}
	if (!isEmergedZuk) {
		zukDeathChance.add(
			rangeGear.equippedWeapon() === getOSItem("Armadyl crossbow"),
			7.5,
			"Zuk with ACB"
		);
		duration.add(
	...
	const usingTbow =
		rangeGear.hasEquipped("Twisted bow", true, true) ||
		rangeGear.hasEquipped("Hellfire bow", true, true);
	zukDeathChance.add(
		usingTbow,
		1.5,
		`Zuk with ${
			usingTbow ? rangeGear.equippedWeapon()?.name : "Twisted bow"
		}`
	);
	duration.add(
		usingTbow,
	 ...
	 */
	duration.add(
		user.bitfield.includes(BitField.HasDexScroll),
		-4,
		"Dex. Prayer scroll"
	);
	duration.add(
		user.bitfield.includes(BitField.HasArcaneScroll),
		-4,
		"Arc. Prayer scroll"
	);
	...
	const isOnTask =
		...
		usersTask.currentTask!.quantity_remaining ===
			usersTask.currentTask!.quantity;

	duration.add(
		isOnTask && user.hasItemEquippedOrInBank("Black mask (i)"),
		-9,
		`${Emoji.Slayer} Slayer Task`
	);
	...
	duration.value = randomVariation(
		duration.value,
		(randInt(1, 10) + randInt(1, 10) + randInt(1, 10)) / 3
	);
	...
	const diedEmergedZuk =
		isEmergedZuk && percentChance(emergedZukDeathChance.value);
	if (diedPreZuk) {
		deathTime = randInt(Time.Minute, calcPercentOfNum(90, duration.value));
	} else if (diedZuk) {
		deathTime = randInt(
			calcPercentOfNum(90, duration.value),
			duration.value
		);
	} else if (diedEmergedZuk) {
		deathTime = randInt(
			calcPercentOfNum(95, duration.value),
			duration.value
		);
	}
...
export async function infernoStatsCommand(user: KlasaUser): CommandResponse {
	...
	const emergedAttempts = user.settings.get(
	...
	let str =
		"You have never attempted the Inferno, I recommend you stay that way.";
	if (attempts && !zukKC) {
...
export async function infernoStartCommand(
	...
	updateBankSetting(
		globalClient,
		ClientSettings.EconomyStats.InfernoCost,
		realCost
	);
...
**Boosts:** ${duration.messages.join(", ")} ${
			duration.missed.length === 0
				? ""
				: `*(You didn't get these: ||${duration.missed.join(", ")}||)*`
		}
**Range Attack Bonus:** ${usersRangeStats.attack_ranged}
**Pre-Zuk Death Chance:** ${preZukDeathChance.value.toFixed(
			1
		)}% ${preZukDeathChance.messages.join(", ")} ${
...
**Zuk Death Chance:** ${zukDeathChance.value.toFixed(
			1
		)}% ${zukDeathChance.messages.join(", ")} ${
