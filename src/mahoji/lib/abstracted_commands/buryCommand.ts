...
export async function buryCommand(user: MUser, channelID: string, boneName: string, quantity?: number) {
	...
	await addSubTaskToActivityTask<BuryingActivityTaskOptions>({
       ...
       return `${user.minionName} is now burying ${cost}, it'll take around ${formatDurationWithTimestamp(duration, user.perkTier(), user.bitfield.includes(BitField.ShowMinionReturnTime))} to finish.`;
}
