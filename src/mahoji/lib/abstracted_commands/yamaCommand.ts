import { calcPerHour, formatDuration, reduceNumByPercent } from '@oldschoolgg/toolkit';
import { Bank, Monsters } from 'oldschooljs';

import { trackLoot } from '@/lib/lootTrack.js';
import killableMonsters from '@/lib/minions/data/killableMonsters/index.js';
import type { AttackStyles } from '@/lib/minions/functions/index.js';
import type { GroupMonsterActivityTaskOptions, YamaTeamMember } from '@/lib/types/minions.js';
...
export async function yamaCommand(
	...
	if (solo) {
		...
		users = await globalClient.makeParty({
			interaction,
			leader: user,
			minSize: 2,
			maxSize: 2,
			ironmanAllowed: true,
			message: `${user.badgedUsername} is looking for one partner to duo Yama. Use the buttons below to join/leave.`,
	...
	const trip = await calcYamaTrip(users, quantity, Boolean(solo));
	...
	return `${user.usernameOrMention}'s ${solo ? 'solo' : 'duo'} Yama trip is starting with ${users
