...
export async function temporossCommand(user: MUser, channelId: string, quantity: number | undefined) {
	...
	return `${user.minionName} is now off to kill Tempoross ${quantity}x times, their trip will take ${await formatTripDuration(user, duration)}. (${formatDuration(durationPerRoss)} per ross)\n\n${messages.join(', ')}.`;
}
