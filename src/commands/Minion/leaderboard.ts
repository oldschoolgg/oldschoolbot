	...
	async skills(msg: KlasaMessage, [inputSkill = 'overall']: [string]) {
		...
		if (inputSkill === 'overall') {
			res = await this.query(
				`SELECT id, "skills.cooking" + "skills.woodcutting" + "skills.mining" + "skills.smithing" + "skills.agility" + "skills.fishing" + "skills.firemaking" + "skills.runecraft" + "skills.crafting" as totalxp FROM users ORDER BY totalxp DESC LIMIT 2000;`
			);
		} else {
			if (!skill) {
