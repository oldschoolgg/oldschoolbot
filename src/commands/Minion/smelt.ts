...
import { SkillsEnum, SmithingActivityEnum } from '../../lib/types';
import { Time, Activity, Tasks, Events } from '../../lib/constants';
import { SmithingActivityTaskOptions } from '../../lib/types/minions';
import addSubTaskToActivityTask from '../../lib/util/addSubTaskToActivityTask';
import Smelting from '../../lib/skills/smithing';
...
export default class extends BotCommand {
    ...
    if (!msg.author.hasMinion) {
		...
		// All bars take 2.4s to smelt, add on quarter of a second to account for banking/etc.
		const timeToSmeltSingleBar = Time.Second * 2.4 + Time.Second / 4;

		// If no quantity provided, set it to the max.
		if (quantity === null) {
			quantity = Math.floor((Time.Minute * 30) / timeToSmeltSingleBar);
		}

		await msg.author.settings.sync(true);
		const userBank = msg.author.settings.get(UserSettings.Bank);

		// Check the user has the required ores to smelt these bars.
		...
		if (duration > Time.Minute * 30) {
			throw `${
				...
				bar.name
			}s you can smelt is ${Math.floor((Time.Minute * 30) / timeToSmeltSingleBar)}.`;
		}
