		...
		*/
		...
		if (minigame === 'dummy') {
			...
			for (const tier of Armours) {
				...
				if (
					(bankHasItem(userBank, requiredWeapons1[0], 1) &&
						bankHasItem(userBank, requiredWeapons1[1], 1)) ||
					(bankHasItem(userBank, requiredWeapons2[0], 1) &&
						bankHasItem(userBank, requiredWeapons2[1], 1))
				) {
					gotCombo = true;
					break;
				}
			...
			if (duration > msg.author.maxTripLength) {
				throw `${msg.author.minionName} can't go on trips longer than ${formatDuration(
					msg.author.maxTripLength
				)}, try a lower quantity. The highest amount of dummies you can whack is ${Math.floor(
					msg.author.maxTripLength / (Time.Second * 7)
				)}.`;
			}
			...
			} is now whacking ${quantity}x dummies, it'll take around ${formatDuration(
				duration
			)} to finish.`;
		...
		if (minigame === 'catapult') {
			...
			if (duration > msg.author.maxTripLength) {
				throw `${msg.author.minionName} can't go on trips longer than ${formatDuration(
					msg.author.maxTripLength
				)}, try a lower quantity. The highest amount of catapult shots that can be blocked is ${Math.floor(
					msg.author.maxTripLength / (Time.Second * 4.5)
				)}.`;
			}
			...
			const response = `${
				msg.author.minionName
			} is now blocking ${quantity}x catapult shots, it'll take around ${formatDuration(
				duration
			)} to finish.`;
		...
		if (minigame === 'shotput') {
			...
			if (duration > msg.author.maxTripLength) {
				throw `${msg.author.minionName} can't go on trips longer than ${formatDuration(
					msg.author.maxTripLength
				)}, try a lower quantity. The highest amount of shot puts that can be thrown is ${Math.floor(
					msg.author.maxTripLength / (Time.Second * 36)
				)}.`;
			}
			...
			const response = `${
				msg.author.minionName
			} is now throwing ${quantity}x shot puts, it'll take around ${formatDuration(
				duration
			)} to finish.`;
		...
		if (minigame === 'jimmy') {
			...
			if (duration > msg.author.maxTripLength) {
				throw `${msg.author.minionName} can't go on trips longer than ${formatDuration(
					msg.author.maxTripLength
				)}, try a lower quantity. The highest amount of kegs that can be balanced is ${Math.floor(
					msg.author.maxTripLength / (Time.Second * 7)
				)}.`;
			}
			...
			const response = `${
				msg.author.minionName
			} is now off to balance ${quantity}x kegs on their head, it'll take around ${formatDuration(
				duration
			)} to finish.`;
