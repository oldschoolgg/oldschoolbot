...
export default class extends BotCommand {
	...
	async run(msg: KlasaMessage, [username, kc = randInt(1, 150)]: [string, number]) {
		...
		ctx.fillText(kc.toString(), 12 + ctx.measureText("Your Kree'arra kill count is: ").width, 10);
		...
		ctx.fillStyle = '#005f00';
		ctx.fillText(
			`${username} received a drop: ${Math.random() > 0.5 ? 'Armadyl hilt' : 'Armadyl chestplate'}`,
			11,
			54
		);
		...
		return msg.channel.send(new MessageAttachment(canvas.toBuffer(), `${Math.round(Math.random() * 10000)}.jpg`));
	}
}
