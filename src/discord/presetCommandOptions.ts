...
export const skillOption = defineOption({
	...
	required: false,
	autocomplete: async ({ value }: StringAutoComplete) => {
		return SkillsArray.filter(i => (!value ? true : i.toLowerCase().includes(value.toLowerCase()))).map(i => ({
			name: toTitleCase(i),
			value: i
		}));
	}
...
export const ownedItemOption = (filter?: (item: Item) => boolean) =>
	defineOption({
		...
		autocomplete: async ({
			value,
			userId
		}: StringAutoComplete): Promise<APIApplicationCommandOptionChoice<string>[]> => {
