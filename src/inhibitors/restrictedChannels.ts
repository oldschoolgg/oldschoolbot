...
import { BitField, PerkTier, SupportServer } from '../lib/constants';
...
export default class extends Inhibitor {
	public async run(msg: KlasaMessage, command: Command) {
		...
		if (msg.guild?.id === SupportServer && !command.restrictedChannels.includes(msg.channel.id)) {
			throw `You cannot use this command outside of one these channels: ${command.restrictedChannels
				.map(c => `<#${c}>`)
				.join(', ')}.`;
		}

		if (msg.guild?.id !== SupportServer && Date.now() - msg.author.createdTimestamp < Time.Year * 2) {
