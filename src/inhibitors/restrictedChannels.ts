...
export default class extends Inhibitor {
	public async run(msg: KlasaMessage, command: Command) {
		...
		if (msg.guild?.id === '342983479501389826' && !command.restrictedChannels.includes(msg.channel.id)) {
			throw `You cannot use this command outside of one these channels: ${command.restrictedChannels
				.map(c => `<#${c}>`)
				.join(', ')}.`;
		}

		if (msg.guild?.id !== '342983479501389826' && Date.now() - msg.author.createdTimestamp < Time.Year * 2) {
