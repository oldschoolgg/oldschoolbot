...
export async function generateGearImage(
	...
	if (
		...
		if (user instanceof KlasaUser ? user.settings.get(UserSettings.Minion.CombatSpell) : user.minion_combatSpell) {
			drawText(
				canvas,
				`Using Spell: ${
					user instanceof KlasaUser
						? user.settings.get(UserSettings.Minion.CombatSpell)
						: user.minion_combatSpell
				}`,
				0,
				16
			);
