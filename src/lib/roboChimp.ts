...
export async function roboChimpSyncData(user: MUser) {
	const [totalClItems, clItems] = await getTotalCl(user, 'collection', await fetchStatsForCL(user));
...
FROM public.user
WHERE osb_cl_percent >= (SELECT osb_cl_percent FROM public.user WHERE id = ${userID});`
	)[0].count;
