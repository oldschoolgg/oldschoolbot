...
import { sortTames } from '@/lib/bso/tames/tameUtil.js';
...
export class MUserClass {
	...
	favAlchs(duration: number, agility?: boolean): Item[] {
		const { bank } = this;
...
Charge your items using ${mentionCommand('minion', 'charge')}.`
	...
	get equippedPet(): Item | null {
		...
		return Items.getOrThrow(this.user.minion_equippedPet);
	}
	...
	async getGodFavour(): Promise<GodFavourBank> {
		const { god_favour_bank: currentFavour } = await this.fetchStats();
		if (!currentFavour) {
			return {
				Zamorak: 0,
				Armadyl: 0,
	...
	async addToGodFavour(gods: GodName[], duration: number) {
		...
		const res = await this.statsUpdate({
	...
	hasGracefulEquipped() {
		for (const gear of Object.values(this.gear)) {
			if (
				gear.hasEquipped('Agility master cape') ||
				gear.hasEquipped(
					[
						'Graceful hood',
						'Graceful top',
	...
	> {
		...
		return { tame: new MTame(tame), activity, species };
	}
