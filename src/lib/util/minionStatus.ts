...
export function minionStatus(user: MUser) {
	...
	switch (currentTask.type) {
		...
		case 'GroupMonsterKilling': {
			...
			return `${name} is currently killing ${data.quantity}x ${monster?.name} with a party of ${data.users.length
				}. ${formattedDuration}`;
		}
		...
		case 'Fishing': {
			...
			return `${name} is currently fishing ${fish?.name}. ${formattedDuration} Your ${Emoji.Fishing
		...
		case 'Nex': {
			...
			const durationRemaining = data.finishDate - data.duration + data.fakeDuration - Date.now();
			return `${name} is currently killing Nex ${data.quantity} times with a team of ${data.users.length
		...
		case 'GiantsFoundry': {
			const data = currentTask as MinigameActivityTaskOptionsWithNoChanges;
			return `${name} is currently creating ${data.quantity
