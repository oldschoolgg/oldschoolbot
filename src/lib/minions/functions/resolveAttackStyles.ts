...
export function resolveAttackStyles({
	...
	if (monster?.id === EBSOMonster.KING_GOLDEMAR) return meleeOnly(inputAttackStyle);
	if (monster?.id === EBSOMonster.VASA_MAGUS) return ['magic'];
	if (monster?.id === NexMonster.id) return ['ranged'];
	if (monster?.id === EBSOMonster.KALPHITE_KING) return meleeOnly(inputAttackStyle);
