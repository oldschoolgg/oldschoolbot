...
export default class extends Task {
	async run({
		...
		const str = `${user}, ${user.minionName} finished throwing ${quantity}x shot puts, You failed ${failedShot} shot puts and received a total of ${tokens}x Warrior guild tokens.`;
		const loot = {
			8851: tokens
		};
