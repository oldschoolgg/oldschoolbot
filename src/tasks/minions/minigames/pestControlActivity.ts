...
import PestControlPoints = UserSettings.PestControlPoints;
...
export default class extends Task {
	async run({
		...
		const points = user.settings.get(PestControlPoints);
		const newPoints =
		...
		await user.settings.update(PestControlPoints, newPoints);
