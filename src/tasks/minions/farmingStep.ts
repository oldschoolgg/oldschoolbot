...
async function applyFarmingLootBoosts({
	user,
	method,
	plant,
	quantity,
	...
	if (user.allItemsOwned.has('Plopper')) {
		...
		boosts.push(`${plopperBoostPercent}% for Plopper`);
	}

	if (user.hasEquippedOrInBank('Farming master cape')) {
		bonusPercentage += 100;
		boosts.push('100% for Farming master cape');
	}
...
async function handlePlantingOnlyStep(options: PlantingOnlyOptions): Promise<FarmingStepResult> {
	...
	const lootBoosts: string[] = [];
	await applyFarmingLootBoosts({
		user,
		method: 'plant',
		plant,
		quantity,
	...
	const summary: FarmingStepSummary = {
		...
		boosts: lootBoosts.length > 0 ? lootBoosts : undefined
	};
...
async function applySpecialFarmingLoot(options: {
	...
	if (plantToHarvest.seedType === 'hespori') {
		...
		if (rng.roll(Math.max(1, plopperDroprate))) {
			loot.add('Plopper');
		}
	} else if (
	...
	if (plantToHarvest.seedType !== 'hespori' && data.duration > Time.Minute * 20 && rng.roll(10)) {
		loot.multiply(2);
...
async function applyBSOPostHarvestLoot(options: {
	...
	if (alivePlants > 0 && plantToHarvest.name === 'Mysterious tree') {
		for (let j = 0; j < alivePlants; j++) {
			const rolls = rng.randInt(1, 3);
			for (let i = 0; i < rolls; i++) {
				loot.add(MysteryBoxes.roll());
			}
		}
	...
	for (const mutation of mutations) {
		if (alivePlants > 0 && plantToHarvest.name === mutation.plantName && rng.roll(mutation.chance)) {
			loot.add(mutation.output);
...
export async function executeFarmingStep({
	...
	const alivePlants = user.allItemsOwned.has('Plopper')
		? patchType.lastQuantity
		: calculateAlivePlants(rng, plantToHarvest, patchType, survivalModifiers);
