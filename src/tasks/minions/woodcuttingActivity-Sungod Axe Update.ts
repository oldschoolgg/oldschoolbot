...
export const woodcuttingTask: MinionTask = {
    ...
    async run(data: WoodcuttingActivityTaskOptions) {
        ...
        const usingSunGodAxe = user.hasEquipped(['Sun-God Axe']);
        const userWcLevel = user.skillLevel(SkillsEnum.Woodcutting);
        ...
        // Add leaves
        if (!usingSunGodAxe && log.leaf && user.hasEquippedOrInBank('Forestry kit')) {
        ...
        if (!usingSunGodAxe && forestry && priffUnlocked && resolveItems(['Teak logs', 'Mahogany logs']).includes(log.id)) {
        ...
        // WC master cape perk
        if (!usingSunGodAxe && user.hasEquippedOrInBank('Woodcutting master cape')) {
            loot.multiply(2);
        }
        ...
        if (clueChance) {
            addSkillingClueToLoot(
                ...
                strungRabbitFoot,
                twitcherSetting,
                usingSunGodAxe ? false : wcCapeNestBoost // Disable nest boost if using Sun-God Axe
            );
        }
        ...
        // Forestry events
        if (forestry) {
            str += await handleForestry({ user, duration, loot: usingSunGodAxe ? new Bank() : loot });
        }
